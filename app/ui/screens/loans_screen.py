...
Updated: date pickers, collateral photo upload, icon labels restored
"""
...
class LoansScreen(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self._collateral_files = []   # list of file paths staged for upload
        self._build()
        self._load_loans()
    ...
    def _render_loan_detail(self, loan):
        ...
        status_colors = {"pending": COLORS["warning"], "approved": COLORS["info"],
        ...
        if coll_data:
            ...
            for i, (desc, fpath) in enumerate(coll_data):
                ...
                if os.path.exists(fpath):
                    ...
                    ctk.CTkLabel(tile, text="DOC", font=FONTS["caption"],
                                 text_color=COLORS["text_muted"]).pack(expand=True)
                ctk.CTkLabel(coll_grid, text=desc[:12],
                             font=FONTS["caption"],
    ...
    def _new_loan_form(self):
        ...
        # Amount + duration
        for key, label in [("principal_amount", "Principal Amount (UGX) *"),
                            ("duration_months", "Duration (months) *")]:
            ctk.CTkLabel(self.detail_panel, text=label,
                         font=FONTS["body_small"],
                         text_color=COLORS["text_secondary"],
        ...
        ctk.CTkFrame(self.detail_panel, fg_color="transparent",
                     height=8).pack()
        ...
        ctk.CTkEntry(self.detail_panel, textvariable=self.purpose_var,
                     **input_style()).pack(fill="x", padx=20)
        ...
        ctk.CTkLabel(self.detail_panel, text="Collateral Documents",
            ...
            anchor="w", padx=20)
        ctk.CTkLabel(self.detail_panel,
                     text="Add photos or scanned documents (land title, logbook, etc.)",
                     font=FONTS["caption"],
        ...
        ctk.CTkButton(coll_btn_row, text="+ Add Photo / Document",
        ...
        self.coll_count_label = ctk.CTkLabel(
            coll_btn_row, text="0 files added",
            font=FONTS["caption"],
            text_color=COLORS["text_muted"])
        self.coll_count_label.pack(side="left", padx=(12, 0))

        # Collateral thumbnails frame
        ...
        ctk.CTkButton(self.detail_panel, text="Submit Loan Application",
    ...
    def _add_collateral(self):
        """Open file dialog to select photos/documents."""
    ...
    def _refresh_collateral_thumbs(self):
        ...
        self.coll_count_label.configure(
            text=f"{len(self._collateral_files)} file(s) added")
        for i, fpath in enumerate(self._collateral_files):
            ...
            # Remove button
            ctk.CTkButton(
                self.coll_thumb_frame, text="x", width=20, height=20,
    ...
    def _update_interest_preview(self, *args):
        try:
            ...
            self.interest_preview.configure(
                text=f"Interest: UGX {interest:,.0f}  |  Total: UGX {total:,.0f}  |  Monthly: UGX {monthly:,.0f}")
        except Exception:
    ...
    def _submit_loan(self):
        ...
        try:
            ...
            # Override the application date with the picked one
            from app.database.connection import get_db
    ...
    def _approve_loan(self, loan_id):
        from app.core.services.loan_service import LoanService
        LoanService.approve_loan(loan_id,
