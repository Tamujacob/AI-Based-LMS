...
Key fixes:
- Principal amount strips commas/spaces before float() conversion
- Input validation with clear error messages on every field
- Decimal conversion is safe at both UI and service layer
"""
...
def _clean_number(raw: str) -> str:
    """Strip commas, spaces, and UGX prefix so float() always works."""
    return raw.strip().replace(",", "").replace(" ", "").replace("UGX", "")


def _to_float(raw: str) -> float:
    """Convert a user-entered number string to float, raising ValueError if invalid."""
...
class FilePicker(tk.Toplevel):
    ...
    def _build(self):
        ...
        self._canvas_window = canvas.create_window((0, 0), window=self.file_frame, anchor="nw")
        ...
        for entry in entries:
            ...
            if is_dir:
                for w in (row_frame, icon_lbl, name_lbl):
                    w.bind("<Double-Button-1>", lambda e, p=entry.path: self._load_dir(p))
                    w.bind("<Button-1>",        lambda e, p=entry.path: self._load_dir(p))
            else:
                for w in (row_frame, icon_lbl, name_lbl):
                    w.bind("<Button-1>", lambda e, p=entry.path: self._toggle_file(p))
    ...
    def _toggle_file(self, path: str):
        ...
        self.sel_label.configure(
            text=f"{count} file{'s' if count != 1 else ''} selected" if count else "No files selected",
...
class LoansScreen(ctk.CTkFrame):
    ...
    def _build_list_panel(self, parent):
        ...
        ctk.CTkLabel(title_row, text="Loans", font=FONTS["title"],
                     text_color=COLORS["accent_green_dark"]).grid(row=0, column=0, sticky="w")
        ...
        self.status_filter = ctk.CTkOptionMenu(
            filter_row,
            values=["All", "pending", "approved", "active", "completed", "defaulted", "rejected"],
    ...
    def _render_loan_detail(self, loan):
        ...
        # Header
        ctk.CTkLabel(
            self.detail_panel, text=loan.loan_number,
            font=FONTS["subtitle"],
        ...
        ctk.CTkLabel(
            ...
            font=FONTS["badge"],
            text_color=status_colors.get(loan.status.value, COLORS["text_secondary"]),
        ...
        if loan.status.value == "pending":
            ctk.CTkButton(
                ...
                command=lambda: self._approve_loan(loan.id),
    ...
    def _render_collateral(self, loan):
        ...
        self._divider()
        ctk.CTkLabel(
            self.detail_panel, text="Collateral Documents",
    ...
    def _new_loan_form(self):
        ...
        ctk.CTkLabel(
            self.detail_panel, text="New Loan Application",
            font=FONTS["subtitle"],
            text_color=COLORS["accent_green_dark"],
        ).pack(anchor="w", padx=20, pady=(20, 4))
        ctk.CTkLabel(
    ...
    def _find_client(self):
        ...
        if clients:
            ...
            self.client_result_label.configure(
                text=(
                    f"✔  {c.full_name}\n"
                    f"NIN: {c.nin or '—'}  |  Phone: {c.phone_number or '—'}"
                ),
    ...
    def _submit_loan(self):
        ...
        try:
            ...
            self.loan_form_error.configure(
                text=f"⚠  Principal amount: {e}  (e.g. enter 500000 or 500,000)")
            return
        if principal <= 0:
            self.loan_form_error.configure(
                text="⚠  Principal amount must be greater than zero.")
            return
        ...
        try:
            ...
            # Update application date if user changed it
            with get_db() as db:
            ...
            with get_db() as db:
                for fpath in self._collateral_files:
                    ...
                    coll = Collateral(
                        loan_id       = loan.id,
                        description   = os.path.splitext(os.path.basename(fpath))[0][:100],
                        file_name     = fname,
                        file_path     = dest,
