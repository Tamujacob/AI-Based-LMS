...
class OpenDialog(tk.Toplevel):
    """
    Themed Open File dialog.
    After wait_window(), check self.result for the chosen path (or None).
    """

    def __init__(self, master, title: str = "Open File",
                 filetypes: list = None):
        super().__init__(master)
        self.result       = None
    ...
    def _build(self, title: str):
        ...
        # File type selector
        if self._filetypes:
            type_text = self._filetypes[0][0] if self._filetypes else "All files"
            tk.Label(bottom, text=f"Type: {type_text}",
                     bg=_LIGHT, fg=_MUTED,
                     font=("Helvetica", 9)).pack(side="left")
    ...
    def _load_dir(self, path: str):
        ...
        for entry in entries:
            ...
            if entry.is_dir():
                ...
                for w in (row_frame, icon_lbl, name_lbl):
                    w.bind("<Button-1>",
                           lambda e, f=entry.name: self._select_file(f))
                    w.bind("<Double-Button-1>",
                           lambda e, f=entry.name: self._confirm_file(f))
                    w.bind("<Enter>",
                           lambda e, f=row_frame: f.configure(bg="#FFF3CD"))
                    w.bind("<Leave>",
                           lambda e, f=row_frame: f.configure(bg=_WHITE))
    ...
    def _select_file(self, filename: str):
        """Select a file (updates the filename field)."""
        self._selected_file = filename
        self.filename_var.set(filename)

    def _confirm_file(self, filename: str):
        """Double-click on file opens it immediately."""
        self._selected_file = filename
        self._confirm()
    ...
    def _confirm(self):
        ...
        # If no path separator, assume it's in current directory
