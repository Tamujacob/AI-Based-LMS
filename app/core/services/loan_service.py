...
class LoanService:
    ...
    def create_loan(
        ...
        Args:
            ...
            principal_amount: Amount in UGX.
        ...
        AuditService.log(
            ...
            description = (
                ...
                f"| Type: {loan_type} "
                f"| Principal: UGX {principal_amount:,.0f} "
                f"| Term: {duration_months} months"
            ),
    ...
    def approve_loan(loan_id: int, approved_by_id: int = None) -> Loan:
        ...
        Args:
            loan_id:          Primary key of the loan to approve.
            approved_by_id:   ID of the approving user (for audit log).
        """
        with get_db() as db:
    ...
    def reject_loan(
        ...
        Args:
            loan_id:          Primary key of the loan to reject.
            reason:           Optional rejection reason.
            rejected_by_id:   ID of the rejecting user (for audit log).
        """
        with get_db() as db:
            loan = db.query(Loan).filter_by(id=loan_id).first()
            if not loan:
    ...
    def mark_completed(loan_id: int, completed_by_id: int = None) -> None:
        ...
        Args:
            loan_id:            Primary key of the loan.
            completed_by_id:    ID of the user triggering completion (for audit log).
        """
        loan_number  = "Unknown"
        client_id    = None
    ...
    def mark_defaulted(loan_id: int, marked_by_id: int = None) -> None:
        ...
        Args:
            loan_id:        Primary key of the loan.
            marked_by_id:   ID of the user marking default (for audit log).
        """
        loan_number = "Unknown"
        client_id   = None
